  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t pb-safe shadow-[0_-10px_30px_rgba(0,0,0,0.3)] bg-[var(--color-card-bg)] border-[var(--color-border)]">
      <div className="flex justify-around items-center px-2 py-3 max-w-md mx-auto h-20 relative">
        ...
        >
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-2xl border-4 transition-all duration-300 border-[var(--color-bg-main)] ${activeTab === 'admin' ? 'bg-[var(--color-accent)] text-[var(--color-bg-main)] rotate-0 scale-110' : 'bg-[var(--color-card-bg)] text-[var(--color-text-secondary)] rotate-45 hover:rotate-0 hover:bg-[var(--color-accent)] hover:text-[var(--color-bg-main)]'}`}>
            <Plus size={32} className={activeTab === 'admin' ? '' : '-rotate-45 group-hover:rotate-0 transition-transform duration-300'} />
          </div>
          <span className={`text-[10px] font-black uppercase tracking-widest mt-2 ${activeTab === 'admin' ? 'text-[var(--color-accent)]' : 'text-[var(--color-text-secondary)]/40'}`}>{t('nav.admin')}</span>
        </motion.button>
