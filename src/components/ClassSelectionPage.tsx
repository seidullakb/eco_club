...
import { GraduationCap, CheckCircle2, Loader2, Users, Sparkles } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[var(--color-bg-main)] flex items-center justify-center p-6 font-sans relative overflow-hidden">
      ...
      >
        ...
        <div className="space-y-10">
          {Object.entries(CLASSES).map(([grade, classes]) => (
              ...
              <div className="grid grid-cols-3 gap-4">
                {classes.map((cls) => (
                  <button
                    ...
                    className={`group relative h-16 rounded-2xl font-black transition-all duration-500 flex items-center justify-center overflow-hidden ${
                      selected === cls 
                  ...
                  >
                    <span className="relative z-10 text-lg">{cls}</span>
                    
                    {/* Hover/Selected Glow */}
                    <div className={`absolute inset-0 bg-gradient-to-tr from-white/20 to-transparent transition-opacity duration-500 ${selected === cls ? 'opacity-100' : 'opacity-0'}`} />
